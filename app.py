...
# [✨정밀 수정] 네이버 검색 데이터에서 종목코드와 이름을 완벽히 발라내는 도우미 함수
def get_stock_code(search_input):
    ...
    try:
        ...
        # 네이버 반환 데이터 구조 파싱 보안 패치
        if 'items' in res and len(res['items']) > 0 and len(res['items'][0]) > 0:
            first_item = res['items'][0][0]
            code = first_item[0][0]  # 종목코드 (예: '005930')
            name = first_item[1][0]  # 종목명 (예: '삼성전자')
            return {"code": code, "name": name}
    except Exception:
        pass
        
    # 기본 백업 매칭 시스템 내장 (검색 API 에러 방지)
    fallback = {
