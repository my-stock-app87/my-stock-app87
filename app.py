...
def analyze(df):
    ...
    # AI 코멘트
    if "상승" in position:
        ai = "거래량 증가와 함께 단기 상승 흐름 유지. 눌림 매수 전략 유효."
    elif "약세" in position:
        ai = "하락 압력 존재. 추가 진입은 위험."
    else:
        ai = "뚜렷한 방향성 없음. 관망이 유리."
...
# ==========================================
    ...
    user_input = st.text_input("종목명 or 코드 입력")
    btn = st.button("분석")
    ...
    if btn and user_input:
        ...
        st.write(f"• [{code}] 거래량 변화 및 변동성 체크 필요")
        st.write("• 기관/외국인 수급 변화 확인 필요")
        st.write("• 최근 5일 추세 기반 단기 방향성 분석")